...
<xsl:stylesheet version="1.0" 
				...
				<table cellpadding="0" cellspacing="0" style="border: none">
					<tr>
						...
						<td style="border: none; vertical-align: top;">
							<xsl:call-template name="get-FeatureFileFilter"/>
						</td>
						<td style="border: none;">
							<h3>Release Version: </h3>
						</td>
						<td style="border: none; vertical-align: top;">
							<xsl:call-template name="get-ReleaseFilter"/>
						</td>
						<td style="border: none; vertical-align: top;">
							<button id="btnClearFilter" onclick="clearFilters();">Clear Filter</button>
						</td>
					</tr>
				</table>
				<table class="reportTable" cellpadding="0" cellspacing="0">
					<tr>
						<th class="top left">Feature File Name</th>
	...
	<xsl:template match="nunit:test-suite" mode="summary">
		...
		<tr class="featureData">
			<xsl:call-template name="get-FeatureFileDivCssTitleName"/>
			<td class="left">
				<xsl:text> </xsl:text>
				<xsl:call-template name="get-FeatureFileCategory"/>
			</td>
			<td class="left">
				<a href="#{$feature-id}">
	...
	<xsl:template match="nunit:test-suite">
		...
		<table class="reportTable" cellpadding="0" cellspacing="0">
			<xsl:call-template name="get-FeatureFileDivCssTitleName"/>
			<tr>				
				<th class="top" style="width: 5em">Scenario PB ID</th>
				<th class="top" style="width: 5em">Release Version</th>
				<th class="top left">
					<xsl:call-template name="get-keyword">
	...
	<xsl:template match="nunit:test-case">
		...
		<tr class="releaseData">
			<xsl:call-template name="get-ReleaseDivCssTitleName"/>
			<td class="left">
				<xsl:text> </xsl:text>
				<xsl:call-template name="get-ScenarioPbIdCategory"/>
			</td>
			<td class="left">
				<xsl:text> </xsl:text>
				<xsl:call-template name="get-ReleaseCategory"/>
			</td>
			<td class="left">
				<xsl:call-template name="get-name"/>
		...
		<xsl:if test="/sfr:NUnitExecutionReport/sfr:ScenarioOutput[@name = $testName]">
			<tr class="releaseData">
				<xsl:call-template name="get-ReleaseDivCssTitleName"/>
				<td id="out{$scenario-id}"  class="left subRow hidden" colspan="5">
		...
		<xsl:if test="$status = 'failure'">
			<tr class="releaseData">
				<xsl:call-template name="get-ReleaseDivCssTitleName"/>
				<td id="err{$scenario-id}" class="left subRow hidden" colspan="5">
	...
	<xsl:template name="get-ReleaseCategory">
		<xsl:choose>
			<xsl:when test="./nunit:categories">
				<xsl:for-each select="./nunit:categories/nunit:category">
					<xsl:if test="starts-with(@name, 'Release')">
						<xsl:value-of select="@name"/>
					</xsl:if>
				</xsl:for-each>
			</xsl:when>
			<xsl:when test="../../@type='ParameterizedTest'">
				<xsl:for-each select="../../nunit:categories/nunit:category">
					<xsl:if test="starts-with(@name, 'Release')">
						<xsl:value-of select="@name"/>
					</xsl:if>
	...
	<xsl:template name="get-ScenarioPbIdCategory">
		<xsl:choose>
			<xsl:when test="./nunit:categories">
				<xsl:for-each select="./nunit:categories/nunit:category">
					<xsl:if test="contains(@name, 'MCC')">
						<xsl:value-of select="@name"/>
					</xsl:if>
				</xsl:for-each>
			</xsl:when>
	...
	<xsl:template name="get-FeatureFileCategory">
		<xsl:choose>
			<xsl:when test="./nunit:categories">
				<xsl:for-each select="./nunit:categories/nunit:category">
					<xsl:if test="contains(@name, '.feature')">
						<xsl:value-of select="@name"/>
					</xsl:if>
				</xsl:for-each>
			</xsl:when>
	...
	<xsl:template name="get-FeatureFileFilter">
		<select id="ddlFeatureFileFilter" name="FeatureFileFilter" onchange="toggleFeatureRows(this);">
	...
	<xsl:template name="get-ReleaseFilter">
		<select id="ddlReleaseFilter" name="ReleaseFilter" onchange="toggleReleaseRows(this);">
			<option value="All">All</option>
			<xsl:choose>
				<xsl:when test="//nunit:categories">
					<xsl:for-each select="//nunit:categories/nunit:category[not(@name=preceding::nunit:category/@name)]">
						<xsl:sort select="@name" order="ascending" />
						<xsl:if test="contains(@name, 'Release')">
							<option>
								<xsl:attribute name="value">
	...
	<xsl:template name="get-FeatureFileDivCssTitleName">
		<xsl:attribute name="title">
			<xsl:choose>
				<xsl:when test="./nunit:categories/nunit:category">
					<xsl:for-each select="./nunit:categories/nunit:category">
						<xsl:if test="contains(@name, '.feature')">
							<xsl:value-of select="substring-before(@name, '.')"/>
						</xsl:if>
	...
	<xsl:template name="get-ReleaseDivCssTitleName">
		<xsl:attribute name="title">
			<xsl:choose>
				<xsl:when test="./nunit:categories">
					<xsl:for-each select="./nunit:categories/nunit:category">
