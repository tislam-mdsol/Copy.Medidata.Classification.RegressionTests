      ...
      <![CDATA[
          function toggle(sdid, event){
            ...
            if (link.innerHTML==showButtonText)
            {
              ...
              toToggle.style.display="table-cell";
            }
            else
            {
		  ...
		  function toggleFeatureRows(element)
		  {
			var tableFeature = document.querySelectorAll(".featureData");
			
			for(var i = 0; i < tableFeature.length; i++) 
			{
				if(element.value == 'All')
				{
					tableFeature[i].style.display = "";
					continue;
				}
				else
				{
					if(tableFeature[i].title.length != 0 )
					{
						if(tableFeature[i].title == element.value)
						{
							tableFeature[i].style.display = "";
						}
						else
						{
							tableFeature[i].style.display = "none";
						}
					}
				}
			}

			toggleReportTableRows(element);		

			if(document.getElementById("ddlReleaseFilter").value != "All")
			{
				toggleFeatureHeaderRows();
			}
		  }
		  
		  function toggleFeatureHeaderRows()
		  {
			var tableReports = document.querySelectorAll(".reportTable");

			for(var i = 0; i < tableReports.length; i++)
			{
				var table = tableReports[i];
				var counter = 0;

				for (var j = 0; j < table.rows.length; j++) 
				{				
					if(table.rows[j].title.length != 0 )
					{
				...
				if((table.rows.length - 1) == counter)
				{
					table.style.display = "none";

					var tableFeature = document.querySelectorAll(".featureData");

					for(var k = 0; k < tableFeature.length; k++)
					{
						if(tableFeature[k].title == table.title)
						{
							tableFeature[k].style.display = "none";
						}
					}
				}
			}			
		  ...
		  function toggleReportTableRows(element)
		  {
			var tableReport = document.querySelectorAll(".reportTable");

			for(var i = 0; i < tableReport.length; i++) 
			{
				if(element.value == 'All')
				{
					tableReport[i].style.display = "";
					continue;
				}
				else
				{
					if(tableReport[i].title.length != 0 )
					{
						if(tableReport[i].title == element.value)
						{
							tableReport[i].style.display = "";
						}
						else
						{
							tableReport[i].style.display = "none";
						}
					}
				}
			}			
		  ...
		  function toggleReleaseRows(element)
		  {
			var tableRelease = document.querySelectorAll(".releaseData");
			
			for(var i = 0; i < tableRelease.length; i++)
			{
				if(element.value == 'All')
				{
					tableRelease[i].style.display = "";
					continue;
				}
				else
				{
					if(tableRelease[i].title.length != 0 )
					{
						if(tableRelease[i].title == element.value)
						{
							tableRelease[i].style.display = "";
						}
						else
						{
							tableRelease[i].style.display = "none";
						}
					}
				}
			}

			toggleFeatureRows(document.getElementById("ddlFeatureFileFilter"));	
		  }

		  function clearFilters()
		  {
			var tableAll = document.querySelectorAll('.featureData, .releaseData, .reportTable');
			
			for(var i = 0; i < tableAll.length; i++)
			{
				if(tableAll[i].style.display == "none")
				{
					tableAll[i].style.display="";
				}
			}

			document.getElementById("ddlFeatureFileFilter").value = "All";
			document.getElementById("ddlReleaseFilter").value = "All";
		  }
          ]]>
    </script>
  </xsl:template>
