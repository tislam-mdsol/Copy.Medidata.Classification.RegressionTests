...
BEGIN
	...
	DECLARE @studyVersionHistory TABLE(DictionaryVersionRefId NVARCHAR(100), ToVersionId NVARCHAR(100), Created DATETIME)

	INSERT INTO @studyVersionHistory(DictionaryVersionRefId, ToVersionId, Created)
	SELECT SMM_From.MedicalDictionaryVersionLocaleKey,
		SMM_To.MedicalDictionaryVersionLocaleKey, 
		SDVH.Created
	...
	INSERT INTO @studyVersionHistory(DictionaryVersionRefId, ToVersionId, Created)
	SELECT SMM.MedicalDictionaryVersionLocaleKey,
		'zz', 
		GETUTCDATE()
	FROM StudyDictionaryVersion SDV
	...
	(
	 SELECT
		...
		WTH.Created,
		WTH.CodingPath,
		SMM.MedicalDictionaryVersionLocaleKey AS CodingPathMedicalDictionaryVersionLocaleKey,
		--ISNULL(WTH.MedicalDictionaryVersionLocaleKey, 0) AS CodingPathMedicalDictionaryVersionLocaleKey,
		E.IsAutoCoded,
		APP.Name AS SourceSystemName,
		SVH.DictionaryVersionRefId AS CurrentMedicalDictionaryVersionLocaleKey
	 FROM tasks E
		INNER JOIN Application APP
			ON APP.SourceSystemID = E.SourceSystemId
		INNER JOIN StudyDictionaryVersion SDV
			ON SDV.StudyDictionaryVersionId = E.StudyDictionaryVersionId
		INNER JOIN SynonymMigrationMngmt SMM
			ON SDV.SynonymManagementID = SMM.SynonymMigrationMngmtID
		...
		( 
			SELECT WTH.*
				,CP.CodingPath, 
				--CP.MedicalDictionaryVersionLocaleKey,  DEV Note: CodingPattern Record will not be populated for Coding Elements that have not been coded
				UH.Login
