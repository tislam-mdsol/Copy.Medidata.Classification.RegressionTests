...
--EXEC spCodingDataForReclassify '5707', 436, 'eng', null, null, null, null, null, null, 1, 1, '','','','',	100, 10, 0, 162, 1, 0, 1, null--, @cnt output

CREATE PROCEDURE dbo.spCodingDataForReclassify (
	@StudyIDs varchar(max), -- comma delimited trackableobjectid's
	
    @VersionLocaleKey NVARCHAR(100),
	...
	@SourceSubject NVARCHAR(100), -- CodingElements.SourceSubject
...
BEGIN
	...
	DECLARE @maxWorkflowTime DATETIME = '1/1/9900'
		...
		SET @SourceSubject           = ISNULL(@SourceSubject          , '')
	...
	;WITH tasksOnly (
		...
		FROM @StudyIDTable STD
			...
			JOIN SynonymMigrationMngmt SMM
				...
				AND @VersionLocaleKey = SMM.MedicalDictionaryVersionLocaleKey
			JOIN CodingElements e 
				...
				AND (@SourceSubject = '' OR e.SourceSubject like '%' + @SourceSubject + '%')
	...
	(
		SELECT
			...
			passesTaskFilter = CASE 
				...
				WHEN dbo.fnIsInDelimitedString(@SkipSegGroupCodingPatternIDs,e.SegmentedGroupCodingPatternID,',')>0 THEN 0
				ELSE 1
			END
		FROM tasksOnly e
	)
