...
BEGIN
	...
	BEGIN TRY
		...
		-- verify project registration is unique
		IF EXISTS (SELECT NULL 
					FROM ProjectDictionaryRegistrations
					WHERE @segmentID = SegmentId
						AND StudyProjectID = @projectID
						AND RegistrationName = @DictionaryRegistration)
		BEGIN
			SET @errorMessage = N'There already exists a registration with this name - Cleanup First!'
			PRINT @errorMessage
			ROLLBACK
