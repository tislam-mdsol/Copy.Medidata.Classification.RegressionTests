...
(
	...
	@AgreementToRunInProduction VARCHAR(255) = '' -- AV - No longer supported, keeping for legacy parameter support
)
AS
BEGIN
	...
	BEGIN
        ...
        PRINT N'Consult with Coder Engineering to find a different solution to the data issues'
		RETURN
	END
	...
	BEGIN
		...
		WHERE StudyDictionaryVersionId IN (SELECT StudyDictionaryVersionId
	...
	BEGIN
		...
		FROM UserObjectRole UOR
			JOIN Roles R
				...
				AND R.ModuleId = (SELECT ModuleId FROM ModulesR WHERE ModuleName = 'PageStudySecurity')
		WHERE UOR.SegmentID = @SegmentID
			AND ISNULL(GrantOnObjectKey, '') <> ''
	...
	PRINT 'Synonym Upload'
	DECLARE @SynonymUploadRequests TABLE 
	(
	   SynonymUploadRequestId INT
	)
	INSERT INTO @SynonymUploadRequests (SynonymUploadRequestId)
	SELECT SynonymUploadRequestId
	FROM SynonymUploadRequests SUR
	JOIN SynonymMigrationMngmt SMM on SMM.SynonymMigrationMngmtID = SUR.SynonymListId
	WHERE SMM.SegmentID = @SegmentID
	
	DELETE FROM SynonymUploadErrors
	WHERE SynonymUploadRequestId IN (SELECT SynonymUploadRequestId FROM @SynonymUploadRequests)
