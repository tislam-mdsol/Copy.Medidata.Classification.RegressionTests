...
BEGIN
	...
	(
	 ...
	 SELECT
		...
		ISNULL(WS.WorkflowStateNameID, 0) AS WorkflowStateNameID,
		ISNULL(WA.NameID, 0) AS WorkflowActionNameID,
		ISNULL(WSAR.ActionName, '') AS WorkflowSystemActionName,
	 ...
	 FROM SearchQueryCTE E
		...
		JOIN WorkflowStates WS
			ON WS.WorkflowStateID = WTH.WorkflowStateID
		LEFT JOIN CodingAssignment CA
		...
		LEFT JOIN WorkflowSystemActionR WSAR
			ON WSAR.WorkflowSystemActionID = WTH.WorkflowSystemActionID
	 OPTION (RECOMPILE)
