...
BEGIN
	...
	DECLARE @studyVersionHistory TABLE(DictionaryVersionRefId INT, ToVersionOrdinal INT, Created DATETIME)

	INSERT INTO @studyVersionHistory(DictionaryVersionRefId, ToVersionOrdinal, Created)
	SELECT DVR.DictionaryVersionRefId,
		SDVH.ToVersionOrdinal, 
		SDVH.Created
	FROM StudyDictionaryVersion SDV
		JOIN StudyDictionaryVersionHistory SDVH
			ON SDVH.StudyId = SDV.StudyId
			AND SDVH.MedicalDictionaryID = SDV.MedicalDictionaryID
			AND SDVH.SegmentID = @SegmentID
		JOIN DictionaryVersionRef DVR
			ON DVR.DictionaryRefID = SDV.MedicalDictionaryID
			AND DVR.Ordinal = SDVH.FromVersionOrdinal
	WHERE StudyDictionaryVersionId = @studyDictionaryVersionId

	DECLARE @maxOrdinal INT

	SELECT @maxOrdinal = ISNULL(MAX(ToVersionOrdinal), 0) + 1
	FROM @studyVersionHistory

	-- insert the current history
	INSERT INTO @studyVersionHistory(DictionaryVersionRefId, ToVersionOrdinal, Created)
	SELECT SDV.DictionaryVersionId,
		@maxOrdinal, 
		GETUTCDATE()
	FROM StudyDictionaryVersion SDV
	WHERE StudyDictionaryVersionId = @studyDictionaryVersionId
