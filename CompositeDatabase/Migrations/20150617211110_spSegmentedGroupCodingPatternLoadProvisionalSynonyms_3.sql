...
(
	@StartDate datetime,
	@EndDate datetime,
	@medicalDictionaryKeys VARCHAR(MAX),
...
AS
	...
	DECLARE @dictionaryKeys TABLE (MedicalDictionaryKey NVARCHAR(100))
	
	INSERT INTO @dictionaryKeys
	SELECT * FROM dbo.fnParseDelimitedString(@medicalDictionaryKeys, @delimiterChar)
	...
	FROM SegmentedGroupCodingPatterns SGCP
		...
		JOIN SynonymMigrationMngmt SMM
			...
			AND CEG.MedicalDictionaryKey IN (SELECT * FROM @dictionaryKeys)
