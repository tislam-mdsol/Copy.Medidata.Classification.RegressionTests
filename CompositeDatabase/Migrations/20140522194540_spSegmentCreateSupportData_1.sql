...
BEGIN
 ...
 -- Check if segments EXIST
	...
	IF NOT EXISTS (SELECT NULL FROM Workflows
		WHERE OID = @SourceWorkflowOID AND SegmentID = @TemplateSegmentID)
	BEGIN
		PRINT N'Workflow does not exist!'
		SET @canProceed = 0
	END
 ...
 -- 3. Workflow Assignment
	...
	SET @workflowName = 'DEFAULT'
	IF NOT EXISTS (SELECT NULL FROM Workflows
		WHERE OID = @workflowName
			AND SegmentID = @NewSegmentID)
	BEGIN
	  	EXECUTE spWorkflowCopy @TemplateSegmentID, @SourceWorkflowOID, @NewSegmentID, @workflowName, 'eng', @workflowID OUTPUT
	END
 
 -- 4. Algorithm Assignment
  -- TODO : must copy all algorithm and assignment structure
  
  
 -- 5. Workflow & Algorithm Defaults
	-- TODO : must insert into objectsegments
