...
BEGIN
	...
	-- find out the default Workflow if not specified
	...
	IF (@DefaultWorkflowID IS NULL)	
	BEGIN
		RAISERROR('DefaultWorkflowID not found', 16, 1)
		RETURN
	END	
	...
	DECLARE @DefaultWorkflowIDString varchar(30), @AllowWorkflowChangeString varchar(30), @IsProdString varchar(30)

	DECLARE
		@AllowWorkflowChange varchar(5), 
		@IsProd varchar(5)

	SET @DefaultWorkflowIDString = 'DefaultWorkflowId'
	SET	@AllowWorkflowChangeString = 'AllowWorkflowChange'
	SET	@IsProdString = 'IsProd'
	SET @AllowWorkflowChange = 'True'
	SET @IsProd = 'False'
	...
	BEGIN
		...
		BEGIN TRY
			...
			EXEC spCreateDefaultDictVerSegmentAssocs @SegmentID, @DictionaryID, @VersionOrdinal, 0, @DefaultWorkflowID, @DefaultAlgorithmID, @Locale, NULL, @dictionaryObTpId, @versionObTypeId
		END TRY
		BEGIN CATCH
		...
		IF (NOT EXISTS (SELECT NULL FROM Workflows
		WHERE WorkflowID =  @DefaultWorkflowID))
		BEGIN
			SET @errorString =  N'Workflow ID:[' + CAST(@DefaultWorkflowID AS VARCHAR) + '] does not exist.'
			RAISERROR(@errorString, 16, 1)
			RETURN
		END
		...
		-- populate the table
		INSERT INTO @AttributesForObject (AttributeDesc, AttributeValue, AlreadyExists)
			VALUES (@DefaultWorkflowIDString, @DefaultWorkflowID, 0)
		INSERT INTO @AttributesForObject (AttributeDesc, AttributeValue, AlreadyExists)
			VALUES (@AllowWorkflowChangeString, @AllowWorkflowChange, 0)
		INSERT INTO @AttributesForObject (AttributeDesc, AttributeValue, AlreadyExists)
