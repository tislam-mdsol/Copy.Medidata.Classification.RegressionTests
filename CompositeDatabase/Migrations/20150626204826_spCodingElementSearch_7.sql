...
CREATE PROCEDURE dbo.spCodingElementSearch (
	...
	@DictionaryVersionLocaleKey NVARCHAR(100), -- 
...
BEGIN
	...
	WHERE SMM.MedicalDictionaryVersionLocaleKey = @DictionaryVersionLocaleKey
		AND SDV.StudyID IN (SELECT * FROM dbo.fnParseDelimitedString(@StudyIDs, ','))
	...
	SELECT 
		...
		CD.CodingPath,
		CD.MedicalDictionaryLevelKey, --actual coded dictionary level
		L.Logline,
		Priority
	FROM @tmpTable SCTE
		-- paths
		CROSS APPLY
		(
			SELECT CD_In.MedicalDictionaryLevelKey,
				CD_In.CodingPath
			FROM SegmentedGroupCodingPatterns SGCP
				CROSS APPLY
				(
					SELECT MedicalDictionaryLevelKey,
						CodingPath
