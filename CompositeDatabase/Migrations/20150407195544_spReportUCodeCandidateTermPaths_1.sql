...
-- Note: this report is currently only supported for English locale.
	...
	DECLARE @DictionaryOID AS VARCHAR(50), @VersionOID AS VARCHAR(50)
	DECLARE @DictionaryVersionID BIGINT
	SELECT @DictionaryOID = DR.OID,	
		@VersionOID = DVR.OID, @DictionaryVersionID = DVR.DictionaryVersionRefID 
	FROM DictionaryVersionRef DVR
		INNER JOIN DictionaryRef DR ON DR.DictionaryRefID = DVR.DictionaryRefID
	WHERE DVR.DictionaryRefID = @MedicalDictionaryID
		AND DVR.Ordinal = @VersionOrdinal
	...
	SELECT TOP(@PageSize)
		...
		@DictionaryOID AS 'Dictionary', @VersionOID AS 'Version', 
