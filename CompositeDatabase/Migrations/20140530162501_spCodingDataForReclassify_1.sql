...
CREATE PROCEDURE dbo.spCodingDataForReclassify (
	...
	@WorkflowVariableName nvarchar(100), -- WorkflowVariables.VariableName
	@WorkflowVariableValue nvarchar(4000), -- WorkflowTaskData.Data
...
BEGIN
	...
	IF (@WorkflowVariableName = '')
		SET @WorkflowVariableName = NULL
	IF (@WorkflowVariableValue = '')
		SET @WorkflowVariableValue = NULL
	...
	;WITH searchQueryCTE (
		...
		WHERE 
			(
			 ...
			 AND 
			(@WorkflowVariableName IS NULL
				OR EXISTS (SELECT NULL
						FROM WorkflowVariables v 
							JOIN WorkflowTaskData d 
								ON v.WorkflowVariableID = d.WorkflowVariableID
								AND d.WorkflowTaskID = e.CodingElementID
								AND v.VariableName = @WorkflowVariableName
						WHERE ( 
								(d.Data IS NULL OR d.Data = '') AND
								@WorkflowVariableValue IS NULL
							   ) OR 
							   d.Data LIKE '%' + @WorkflowVariableValue + '%'
			 ))
			 AND NOT EXISTS 
	...
	WHERE RowNumber BETWEEN @startRowNumber AND @endRowNumber
		...
		JOIN WorkflowStates WS 
			ON WS.WorkflowStateID = XI.WorkflowStateID
		JOIN Workflows w 
		...
		(
			...
			FROM CodingElements CE
				JOIN WorkflowStates WS
					...
					AND WS.IsReconsiderState = 0
					AND CE.AssignedSegmentedGroupCodingPatternID = GD.SegmentedGroupCodingPatternID
