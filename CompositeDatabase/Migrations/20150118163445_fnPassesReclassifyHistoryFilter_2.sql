...
BEGIN
	...
	IF EXISTS (
					...
					WHERE WTH.WorkflowTaskId = @CodingElementID AND
						...
						AND (@PriorWorkflowStateIDs = '' OR CHARINDEX(CAST(WTH.WorkflowStateID AS VARCHAR)+',', @PriorWorkflowStateIDs) > 0)
						-- 3. prioraction check
						AND (@PriorWorkflowActionIDs = '' OR CHARINDEX(CAST(WTH.WorkflowActionID AS VARCHAR)+',', @PriorWorkflowActionIDs) > 0)
					)
