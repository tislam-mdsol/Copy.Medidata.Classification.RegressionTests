...
BEGIN
	...
	BEGIN
		...
		SELECT CE.CodingElementId,
			CASE WHEN dbo.fnLDS(WS.WorkflowStateNameID, 'eng') = 'Waiting Manual Code' THEN 1 ELSE 0 END,
			CASE WHEN dbo.fnLDS(WS.WorkflowStateNameID, 'eng') = 'Waiting Approval' THEN 1 ELSE 0 END,
		...
		(
			...
			JOIN WorkflowStates WS
				ON WS.WorkflowStateID = CE.WorkflowStateID
			JOIN WorkflowVariables WV_AutoCode
				ON WV_AutoCode.WorkflowID = WS.WorkflowID
				AND WV_AutoCode.VariableName = 'IsAutoCode'	
			JOIN WorkflowVariables WV_AutoApprove
				ON WV_AutoApprove.WorkflowID = WS.WorkflowID
