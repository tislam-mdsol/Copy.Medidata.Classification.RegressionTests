...
AS
    ...
    DECLARE @ResultIds TABLE (RowNUM INT, SegmentedGroupCodingPatternID INT)
    ...
    SELECT SGCP.* 
        FROM SegmentedGroupCodingPatterns SGCP
            INNER JOIN @ResultIds IDS
                ON SGCP.SegmentedGroupCodingPatternID = IDS.SegmentedGroupCodingPatternID
		WHERE IDS.RowNUM BETWEEN @startRowNumber AND @endRowNumber
