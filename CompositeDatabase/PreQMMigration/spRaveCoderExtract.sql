...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'spRaveCoderExtract')
	DROP PROCEDURE dbo.spRaveCoderExtract
GO
CREATE PROCEDURE dbo.spRaveCoderExtract
(
    @RaveToken INT 
)
AS
BEGIN
    ...
    INSERT INTO RaveCoderExtract 
		(ExternalObjectName,EnvironmentName,ExternalObjectId,SiteName,SiteNumber,SourceSubject,FolderName,FolderOID,ParentInstance,InstanceName,InstanceRepeatNumber,SourceForm,FormOID,DataPageName,PageRepeatNumber,SourceField,
		RecordPosition,UUID,DatapointID,HasContexthash,CodingContextURI,Locale,RegistrationName,Dataactive,Istouched,RecordActive,StudyActive,StudysiteActive,SubjectActive,projectactive,VerbatimTerm,SupplementalTermkey,
		SupplementalValue,CodedDate,IsCoded,Codingpath,Created,RaveToken)
    select distinct
    ...
    -- Active/deleted flags END
	    replace(replace(replace(dpt.data,char(10),' '),char(13),' '),char(9),' ')  as VerbatimTerm,
    ...
    from
		...
		join coderconfigurations cc on cc.fieldid = fi.fieldid
		join codingcolumns ccl on ccl.codingcolumnid = cc.codingcolumnid
		join codingdictionaries cdict on cdict.codingdictionaryid = ccl.codingdictionaryid
	    cross apply ( 
	        select 
			    (select ':'+fo2.oid+'.'+fi2.oid as keys , 
					    ':'+ case when dde.UserDataStringID is null then replace(replace(replace(dpt2.data,char(10),' '),char(13),' '),char(9),' ')
